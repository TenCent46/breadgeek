...
export function IngredientsClient({ initialIngredients }: IngredientsClientProps) {
  ...
  async function handleSaveEdit() {
    ...
    // Optimistic update
    setIngredients((prev) =>
      prev.map((ing) => (ing.id === selected.id ? { ...ing, ...updates } : ing))
    );
    setSelected(null);

    await updateIngredientAction(selected.id, updates);
  }
  ...
  async function handleDeleteIngredient() {
    ...
    // Optimistic update
    setIngredients((prev) => prev.filter((ing) => ing.id !== deletedId));
    setSelected(null);

    await deleteIngredientAction(deletedId);
  }
  ...
  async function handleAdd() {
    ...
    const optimisticIngredient: Ingredient = {
      ...
      costHistory: [{ date: new Date().toISOString().slice(0, 10), unitCostPerKg: addUnitCost }],
      createdAt: new Date().toISOString().slice(0, 10),
      updatedAt: new Date().toISOString().slice(0, 10),
    };
    setIngredients((prev) => [...prev, optimisticIngredient]);
    setShowAdd(false);

    await addIngredientAction(newData);
  }
