...
import type { Service, ServiceType, ServiceStatus, ServiceSchedule, Recipe } from "@/lib/types";
...
export function ServiceEditClient({ service, recipes }: ServiceEditClientProps) {
  const router = useRouter();

  // Form state initialized from service prop
  ...
  const [capacity, setCapacity] = useState<number>(service.capacity || 6);
  const [duration, setDuration] = useState<number>(service.duration || 120);
  const [location, setLocation] = useState(service.location || "");
  const [schedules, setSchedules] = useState<ServiceSchedule[]>(service.schedules);
  const [cancellationPolicy, setCancellationPolicy] = useState("");
  const [linkedRecipeId, setLinkedRecipeId] = useState(service.linkedRecipeId || "");
